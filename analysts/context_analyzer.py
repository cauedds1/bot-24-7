...
def verificar_veto_mercado(tipo_aposta, script_name):
    """
    LAYER 3 (PHOENIX V2.0): Sistema de VETO - Verifica se uma aposta é VETADA pelo script tático.
    
    Previne apostas absurdas como "Under 2.5" quando casa domina.
    
    Args:
        tipo_aposta: String do tipo de aposta (ex: "Over 2.5", "Under 2.5", "Vitória Casa")
        script_name: Nome do script tático atual
    
    Returns:
        tuple: (is_vetado: bool, razao: str)
    """
    if script_name not in MERCADOS_VETADOS_POR_SCRIPT:
        return False, None
    
    script_config = MERCADOS_VETADOS_POR_SCRIPT[script_name]
    mercados_vetados = script_config['vetados']
    
    # Normalizar tipo_aposta para comparação (remover espaços extras, case-insensitive)
    tipo_normalizado = tipo_aposta.strip()
    
    # Verificar se o tipo de aposta está na lista de vetados
    for mercado_vetado in mercados_vetados:
        if mercado_vetado.lower() in tipo_normalizado.lower():
            return True, script_config['razao']
    
    return False, None


def ajustar_confianca_por_script(confianca_base, tipo_aposta, script_name):
    """
    LAYER 4 (PHOENIX V2.0): Ajusta confiança baseado em COERÊNCIA com o script tático.
    
    Aumenta confiança se a aposta é COERENTE com o script.
    Reduz confiança se a aposta é INCOERENTE com o script.
    
    Args:
        confianca_base: Confiança calculada matematicamente (5.0-9.5)
        tipo_aposta: Tipo de aposta (ex: "Over 2.5", "Vitória Casa")
        script_name: Nome do script tático
    
    Returns:
        float: Confiança ajustada (5.0-9.5)
    """
    ajuste = 0.0
    
    # Scripts de DOMÍNIO - Coerência: gols e vitória do dominante
    if script_name in ['SCRIPT_DOMINIO_CASA', 'SCRIPT_HOST_DOMINATION', 'SCRIPT_TIME_EM_CHAMAS_CASA']:
        if 'Over' in tipo_aposta or 'Vitória Casa' in tipo_aposta or 'Casa Vence' in tipo_aposta:
            ajuste = +1.0  # MUITO coerente
        elif 'Under' in tipo_aposta or 'Fora' in tipo_aposta:
            ajuste = -1.5  # INCOERENTE
    
    elif script_name in ['SCRIPT_DOMINIO_VISITANTE', 'SCRIPT_TIME_EM_CHAMAS_FORA']:
        if 'Over' in tipo_aposta or 'Vitória Fora' in tipo_aposta or 'Fora Vence' in tipo_aposta:
            ajuste = +1.0  # MUITO coerente
        elif 'Under' in tipo_aposta or 'Casa' in tipo_aposta and 'Vence' in tipo_aposta:
            ajuste = -1.5  # INCOERENTE
    
    # Scripts de JOGO ABERTO - Coerência: Over gols e BTTS
    elif script_name in ['SCRIPT_OPEN_HIGH_SCORING_GAME', 'SCRIPT_MATA_MATA_VOLTA']:
        if 'Over' in tipo_aposta or 'BTTS' in tipo_aposta and 'Sim' in tipo_aposta:
            ajuste = +0.8  # Coerente
        elif 'Under 2.5' in tipo_aposta or 'Under 1.5' in tipo_aposta:
            ajuste = -1.0  # Incoerente
    
    # Scripts de JOGO TRAVADO - Coerência: Under gols
    elif script_name in ['SCRIPT_CAGEY_TACTICAL_AFFAIR', 'SCRIPT_RELEGATION_BATTLE', 'SCRIPT_JOGO_DE_COMPADRES']:
        if 'Under' in tipo_aposta:
            ajuste = +0.8  # Coerente
        elif 'Over 3.5' in tipo_aposta or 'Over 4.5' in tipo_aposta:
            ajuste = -1.0  # Incoerente
    
    # Aplicar ajuste
    confianca_ajustada = confianca_base + ajuste
    
    # Limitar entre 5.0 e 9.5
    confianca_final = max(5.0, min(9.5, confianca_ajustada))
    
    if ajuste != 0:
        sinal = '+' if ajuste > 0 else ''
        print(f"    🎯 SCRIPT COHERENCE: {tipo_aposta} com {script_name} → Ajuste: {sinal}{ajuste:.1f} (Confiança: {confianca_base:.1f} → {confianca_final:.1f})")
